...
def up(revision: str) -> None:
    ...
    try:
        subprocess.check_call(
            f"alembic -c {ALEMBIC_CONFIG_FILE} upgrade {revision}", shell=True
        )
    except subprocess.CalledProcessError as exc:
        exit(exc.returncode)
...
def down(revision: str) -> None:
    ...
    try:
        subprocess.check_call(
            f"alembic -c {ALEMBIC_CONFIG_FILE} downgrade {revision}", shell=True
        )
    except subprocess.CalledProcessError as exc:
        exit(exc.returncode)
...
def _validate_revision(revision: str) -> None:
    ...
    if " " in revision.strip():
        raise click.BadArgumentUsage(
            ...
            "Revision must be one word without any spaces."
        )
